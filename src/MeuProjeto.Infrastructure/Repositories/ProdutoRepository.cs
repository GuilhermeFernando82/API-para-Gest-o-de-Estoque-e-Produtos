...
public class ProdutoRepository : IProdutoRepository
{
    ...
    {
        using var conn = _connectionFactory.CreateConnection();
        await conn.ExecuteAsync(@"INSERT INTO produtos (id, nome, descricao, preco) VALUES (@Id, @Nome, @Descricao, @Preco)", produto);
    }

    public async Task AtualizarAsync(Produto produto)
    {
        using var conn = _connectionFactory.CreateConnection();
        await conn.ExecuteAsync(@"UPDATE produtos SET nome = @Nome, descricao = @Descricao, preco = @Preco WHERE id = @Id", produto);
    }
